...
void Game::ShowdebugInfo()
{
    ...
    for (map::Tile tile : map.tilesArray)
    {
        if (debug)
        {
            DrawText(tile.GetLabel(), tile.WorldX() + 10, tile.WorldY() + 10, 12, BLACK);
...
{
    money += refunds;
    map.GetTile(turrets[itemSelected]->x/TILES_SIZE, turrets[itemSelected]->y/TILES_SIZE).free = true;
...
{
    // When erasing, prefer to use iterators (begin(), end())
    for (auto it = explosions.begin(); it != explosions.end(); )
    {
        Explosion& explosion = *it;
        explosion.Play(ressources.explosionTexture);
        if (explosion.animationDone) // erase
...
void Game::PlayGameTick()
{
    ...
    if (gameState == GameState::GM_ISRUNNING || gameState == GameState::GM_ISPAUSED || gameState == GameState::GM_GAMEOVER)
    {
        ...
        if (ui::Button(SCREEN_WIDTH - 250, 720, 40, 40, 40, "||", GRAY) || IsKeyPressed(KEY_P))
        {
        ...
        else if (gameState == GameState::GM_ISPAUSED)
        {
            menu.shouldDisplay = true;
